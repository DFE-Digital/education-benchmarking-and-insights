...
import { TotalTeachersData } from "src/views/compare-your-census-2/partials";
...
export const TotalTeachers: React.FC<{ type: string; id: string }> = ({
  ...
  const [data, setData] = useState<TotalTeachersData[] | null>();
  ...
  useEffect(() => {
    const merged = censusData
      ? censusData.reduce<TotalTeachersData[]>(
          (acc: TotalTeachersData[], curr: Census) => {
            acc.push({
              ...curr,
