...
import { NonClassroomSupportData } from "src/views/compare-your-census-2/partials";
...
export const NonClassroomSupport: React.FC<{ type: string; id: string }> = ({
  ...
  const [data, setData] = useState<NonClassroomSupportData[] | null>();
  ...
  useEffect(() => {
    const merged = censusData
      ? censusData.reduce<NonClassroomSupportData[]>(
          (acc: NonClassroomSupportData[], curr: Census) => {
            acc.push({
              ...curr,
