...
FBIT have agreed that the refresh of the calculations in the LAA should be done at the same time as the CFR data refresh is done in the main service. All the ancillary data for LAA refreshes in time to do this, and this refresh time brings the data refresh of LAA in line with FBIT as a whole for consistency of experience across the service.
...
* Minimise impact on existing FBIT query/view latency. In some cases, e.g. National Averages, our views are quite nested and have required caching to reduce latency.
...
### Schema option C: Store none of the 6 new raw datapoints in the database; add denormalised tables for headline and breakdown risk indicators
...
This option does not persist the 6 new raw non-financial datapoints in the database. QA would be done on input data files, a parquet saved from the data pipeline at the time of calculation, and the data download file available on the risk indicator webpage (this would contain data as well as risk scores).

## Decision Outcome

The recommendation is to proceed with **Trigger option B: Custom trigger for LAA processing** and **Schema option C: denormalised tables for headline and breakdown risk indicators, with no persistence of the 6 new raw datapoints**.

### Rationale

This option fulfils our problem statement. It is extensible, versions risk score derivations over time, has no impact on existing FBIT view latency, has no impact on the run time of the main data pipeline, provides a simple view interface to the new webpages (which can easily handle new risk categories), and maintains the ability to QA figures, albeit via files rather than the database.
