...
School, Trust, and Local Authority entities are displayed in their raw state as provided by the upstream data; as a result, inconsistent casing and irregular font weights are observed within the service. These styles are preserved to match the system of record, and no cleansing logic is applied to avoid the risk of breaking specific acronyms or nomenclature. Additionally, irregular font weights are retained from the original legacy data feeds, ensuring that no data loss or formatting errors occur during ingestion.
