...
15. Copy the values from `PivotTables` sheet into the relevant section in `Summary`
   > **NOTE:**
   > 
   > For School Engagement summary, `FBIT Total Schools (all time)` and `Percentage of FBIT engaged` are calculated rows, do not overwrite but update formular if required.
   > 
   > For CFP completion summary, `Total plans created` and `# Schools with plans` are calculated rows, do not overwrite but update formular if required.
...
18. Repeat steps 14 and 15 to load `TrustFeatures`, `LAFeatures` and `SfpData` repectively.
19. Copy the loaded data exculding headers and paste values to overwrite the respective existing summary report.
20. Upon overwrite completion, delete the loaded data (colmns H to J)
...
_To integrate, update or change log analytics query in Excel see [Refrence](https://learn.microsoft.com/en-us/azure/azure-monitor/logs/log-excel)_

_To integrate, update or change SQL query in Excel see [Refrence](https://learn.microsoft.com/en-us/power-query/power-query-ui)_
