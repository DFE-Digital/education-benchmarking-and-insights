...
This plan defines the QA strategy to validate the Data Release for the **Budget Forecast Return (BFR) covering the 2025-2026 period**.
...
The BFR schema is unchanged from the previous (2024-2025) release, so this cycle is a like-for-like release of the established pipeline with the reporting year advanced to 2026.
...
- **Service/UI Validation:** Functional verification of the FBIT front-end for the 2026 reporting year, confirming visibility for trusts and schools.
- **Regression Testing:** Verification that historical BFR data (2025 and prior) remains unchanged during this data release.

### Out of Scope

- Validation of the accuracy of data within the raw source files.
...
| **Trust linkage** | `Trust` and `TrustFinancial` database exports (Trust UPIN to Company Registration Number mapping and trust pupil numbers) |
...
- **Logic Verification:** Review any specific transformation or aggregation logic applied to the SOFA and three-year forecast data (scaling, EFA line filtering, category aggregation, and the derived percentage metrics) to ensure it aligns with business rules.

### Database Validation

**Goal:** Ensure data persistence and relational integrity.

- **Data Reconciliation:** Query the database for 2026 BFR data and confirm row counts match source file totals.
...
- **Comparative Analysis:** Compare historical row counts and sample records before and after the 2026 ingestion cycle.
...
| Role | Responsibility |
| :--- | :--- |
| **Data Analyst(s)** | Produce and review source BFR files for the data release. |
...
- [ ] Regression tests confirm historical BFR data integrity (2025 and prior)
- [ ] UI/Service displays 2026 BFR data accurately with correct metrics
