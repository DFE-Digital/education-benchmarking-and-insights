...
- Computational Validation: Perform manual "calculator-style" verification for a sample of records using the logic provided in the transparency calculation.
- Accuracy Benchmarking:** Compare manually computed transparency metrics against the values generated by the automated script; ensure zero variance between the two.
- Logic Compliance:** Confirm that the output matches the specific business rules and formulas defined for the AAR 2025 transparency release.
