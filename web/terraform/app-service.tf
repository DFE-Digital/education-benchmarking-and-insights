...
resource "azurerm_windows_web_app" "education-benchmarking-as" {
  ...
  app_settings = {
    ...
    "FeatureManagement__HighNeedsBenchmarking"              = var.configuration[var.environment].features.HighNeedsBenchmarking
    "FeatureManagement__SchoolComparisonFilter"             = var.configuration[var.environment].features.SchoolComparisonFilter
    "FeatureManagement__LocalAuthorityRiskIndicators"       = var.configuration[var.environment].features.LocalAuthorityRiskIndicators
    ...
    "Apis__Trust__Key"                                      = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.trust-api-key.versionless_id})"
    "Apis__LocalAuthority__Url"                             = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.local-authority-api-host.versionless_id})"
    "Apis__LocalAuthority__Key"                             = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.local-authority-api-key.versionless_id})"
    ...
    "ClarityOptions__ProjectId"                             = try(var.configuration[var.environment].ClarityOptions.ProjectId, null)
  }
  tags = local.common-tags
}
