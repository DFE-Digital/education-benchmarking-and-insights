...
# this policy will be tidied up in a future release
...
# new policy, intentionally orphaned in the initial release
resource "azurerm_cdn_frontdoor_firewall_policy" "web-app-front-door-waf" {
  ...
  dynamic "managed_rule" {
    for_each = (var.configuration[var.environment].front_door_waf_policy_sku_name == "Premium_AzureFrontDoor" ?
    ["apply"] : [])
    content {
      type    = "DefaultRuleSet"
      version = "1.0"
