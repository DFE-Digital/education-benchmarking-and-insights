...
[Binding]
[Scope(Feature = "Trust historic data")]
public class HistoricDataSteps(PageDriver driver)
{
    private HistoricDataPage? _historicDataPage;

    [Given("I am on '(.*)' history page for trust with company number '(.*)'")]
    public async Task GivenIAmOnHistoryPageForTrustWithCompanyNumber(string tab, string companyNumber)
    {
        var url = HistoricDataUrl(tab, companyNumber);
