...
[Binding]
[Scope(Feature = "Local Authority benchmark census data")]
public class BenchmarkCensusSteps(PageDriver driver)
{
    ...
    [Given("I am on census page for local authority with code '(.*)'")]
    public async Task GivenIAmOnCensusPageForLocalAuthorityWithCode(string companyNumber)
    {
    ...
    private static string BenchmarkCensusUrl(string code) => $"{TestConfiguration.ServiceUrl}/local-authority/{code}/census";
}