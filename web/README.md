...
The `web` module is the primary user-facing portal for the Education Benchmarking and Insights platform.

### Tech Stack

- **Backend:** .NET 10 (ASP.NET Core MVC)
- **Frontend:** Vue 3 (Composition API), Pinia, TypeScript, Vite, GOV.UK Frontend.
...
- **`Web.App`**: Core application (Controllers, domain logic, Vue assets).
- **`Web.Identity`**: Authentication and DfE Sign-in (OIDC) integration.
- **`Web.Redirect`**: legacy or vanity URL redirections.
- **`Web.Shutter`**: Maintenance mode and outages.
