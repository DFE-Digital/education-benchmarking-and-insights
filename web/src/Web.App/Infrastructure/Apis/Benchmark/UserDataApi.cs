...
{
    public async Task<ApiResult> GetAsync(ApiQuery? query = null, CancellationToken cancellationToken = default) => await GetAsync($"{Api.UserData.All}{query?.ToQueryString()}", cancellationToken);
}
