...
public class ComparatorSetApi(HttpClient httpClient, string? key = default) : ApiBase(httpClient, key), IComparatorSetApi
{
    ...
    public async Task<ApiResult> UpsertUserDefinedSchoolAsync(string urn, PostComparatorSetUserDefinedRequest request) => await PostAsync(Api.ComparatorSet.SchoolUserDefined(urn, null), new JsonContent(request));
