...
import { SeniorLeadershipChartTooltip } from "@/main";
import type {
  FocusSource, SeniorLeadershipChartTooltipsProps, SeniorLeadershipChartTooltipPropsData
} from ".";
import { onMounted, onUnmounted, ref } from "vue";

const { data } = defineProps<SeniorLeadershipChartTooltipsProps>();
const tooltipX = ref(0);
const tooltipY = ref(0);
const datum = ref<SeniorLeadershipChartTooltipPropsData | null>(null);
const focusSource = ref<FocusSource>();
const keyAttribute = "data-key";

const eventListenerMouseEnter = (e: MouseEvent, d: SeniorLeadershipChartTooltipPropsData) => {
...
const eventListenerElementFocus = (e: Event, d: SeniorLeadershipChartTooltipPropsData) => {
...
const addMouseListeners = (el: SVGGeometryElement, d: SeniorLeadershipChartTooltipPropsData) => {
  el.addEventListener("mouseenter", (e) => eventListenerMouseEnter(e, d));
  el.addEventListener("mouseleave", eventListenerExit);
};

const addFocusListeners = (el: Element, d: SeniorLeadershipChartTooltipPropsData) => {
  el.addEventListener("focus", (e) => eventListenerElementFocus(e, d));
  el.addEventListener("blur", eventListenerExit);
};

const removeMouseListeners = (el: SVGGeometryElement, d: SeniorLeadershipChartTooltipPropsData) => {
  el.removeEventListener("mouseenter", (e) => eventListenerMouseEnter(e, d));
  el.removeEventListener("mouseleave", eventListenerExit);
};

const removeFocusListeners = (el: Element, d: SeniorLeadershipChartTooltipPropsData) => {
...
defineOptions({
  name: "SeniorLeadershipChartTooltips",
});
</script>

<template>
  <Teleport to="body">
    <SeniorLeadershipChartTooltip :datum="datum" :x="tooltipX" :y="tooltipY" :focus-source="focusSource" />
  </Teleport>
</template>
