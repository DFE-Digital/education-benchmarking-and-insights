...
function OrganisationInput(props) {
    let { input, identifier, kind } = props;
    ...
    function fillSearch() {
        ...
        setSelectedIdentifier(options[index].document.identifier)
        setSelectedKind(options[index].document.kind)
    ...
    return <div className="suggest-input">
        <input id="organisation-input" ref={organisationInput} className="govuk-input" aria-label="organisationInput"
               name="input" type="text"
               value={inputValue} onKeyDown={onKeyDown} onKeyUp={onKeyPressed} onBlur={hideOptions} onChange={onChange}
               autoComplete="off"/>
        <input value={selectedIdentifier} name="identifier" type="hidden"/>
        <input value={selectedKind} name="kind" type="hidden"/>
        {options.length > 0 && optionsVisible && <ul className="govuk-input" id="organisation-suggestions">
