    ...
    ILogger<TrustComparisonItSpendController> logger) : Controller
{
        ...
        Views.ViewAsOptions viewAs = Views.ViewAsOptions.Chart)
    {
        ...
        }))
        {
            try
            {
                ...
                var (userDataComparatorSet, userDefinedSet, hasTrustAuthorisation) = await GetTrustUserContext(companyNumber);
    ...
    private async Task<(string? userDataComparatorSet, UserDefinedSchoolComparatorSet? userDefinedSet, bool hasTrustAuthorisation)> GetTrustUserContext(string companyNumber)
    {
