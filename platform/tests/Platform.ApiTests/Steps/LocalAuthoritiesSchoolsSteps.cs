...
{
    private const string SchoolsFinanceKey = "schools-finance-key";

    [Given("a local authorities schools request with code '(.*)' and query parameters:")]
    public void GivenALocalAuthoritiesSchoolsRequestWithCodeAndQueryParameters(string code, DataTable table)
    {
        var queryString = BuildQueryString(table);

        api.CreateRequest(SchoolsFinanceKey, new HttpRequestMessage
        {
    ...
    {
        var response = api[SchoolsFinanceKey].Response;
