Feature: Chart rendering vertical bar chart endpoint
    ...
    Scenario: Sending a valid single vertical bar chart request returns the correct HTML
        Given a single vertical bar chart request with accept header 'application/json', highlighted item '123456', sort 'asc', width '500', height '100', id 'test-uuid' and the following data:
          | Key    | Value   |
          | 123450 | 1000001 |
          | 123451 | 1000111 |
          | 123452 | 1000011 |
        ...
        Then the response should be ok, contain a JSON object and match the expected output of 'VerticalBarChartValidSingle.json'
    ...
    Scenario: Sending a valid single vertical bar chart request with accept header image/svg+xml returns the correct HTML only
        Given a single vertical bar chart request with accept header 'image/svg+xml', highlighted item '123456', sort 'asc', width '500', height '100', id 'test-uuid' and the following data:
          | Key    | Value   |
          | 123450 | 1000001 |
          | 123451 | 1000111 |
          | 123452 | 1000011 |
          ...
          | 123459 | 1100001 |
        When I submit the vertical bar chart request
        Then the response should be ok, contain an SVG document and match the expected output of 'VerticalBarChartValidSingle.svg'
    ...
    Scenario: Sending a valid multiple vertical bar chart request returns the correct HTML
        ...
        When I submit the vertical bar chart request
        Then the response should be ok, contain a JSON array and match the expected output of 'VerticalBarChartValidMultiple.json'
