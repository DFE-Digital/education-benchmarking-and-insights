Feature: Insights census endpoints
    ...
    Scenario: Sending a valid school census history request
        ...
        Then the census history result should be ok and contain:
          | Year | TotalPupils | Workforce | WorkforceHeadcount | Teachers | SeniorLeadership | TeachingAssistant | NonClassroomSupportStaff | AuxiliaryStaff | PercentTeacherWithQualifiedStatus |
          | 2022 | 337.00      | 29.00     | 33.00              | 116.00   | 16.00            | 1.00              | 0.00                     | 3.00           | 1.00                              |
    ...
    Scenario: Sending a valid school average across comparator set census history request with dimension Total
        ...
        Then the census history result should be ok and contain:
          | Year | TotalPupils | Workforce | WorkforceHeadcount | Teachers   | SeniorLeadership | TeachingAssistant | NonClassroomSupportStaff | AuxiliaryStaff | PercentTeacherWithQualifiedStatus |
          | 2022 | 268.633333  | 47.800000 | 56.233333          | 124.066666 | 26.933333        | 1.000000          |                          | 3.566666       | 1.033333                          |
    ...
    Scenario: Sending a valid school average across comparator set census history request with dimension HeadcountPerFte
        ...
        When I submit the insights census request
        Then the census history result should be ok and contain:
          | Year | TotalPupils | Workforce             | WorkforceHeadcount    | Teachers              | SeniorLeadership      | TeachingAssistant      | AuxiliaryStaff        | PercentTeacherWithQualifiedStatus |
          | 2022 | 268.633333  | 1.2454354735846555320 | 1.0000000000000000000 | 0.0090565029104613964 | 0.1168831168831168830 | 25.2000000000000000000 | 1.0492063492063492063 | 1.033333                          |
    ...
    Scenario: Sending a valid school average across comparator set census history request with dimension PercentWorkforce
        ...
        Then the census history result should be ok and contain:
          | Year | TotalPupils | Workforce              | WorkforceHeadcount     | Teachers               | SeniorLeadership      | TeachingAssistant    | AuxiliaryStaff        | PercentTeacherWithQualifiedStatus |
          | 2022 | 268.633333  | 100.000000000000000000 | 100.000000000000000000 | 490.534045780128272232 | 50.793106165302500259 | 3.884367879357122409 | 10.195699860404623272 | 1.033333                          |
    ...
    Scenario: Sending a valid school average across comparator set census history request with dimension PupilsPerStaffRole
        ...
        Then the census history result should be ok and contain:
          | Year | TotalPupils | Workforce             | WorkforceHeadcount    | Teachers              | SeniorLeadership       | TeachingAssistant       | AuxiliaryStaff         | PercentTeacherWithQualifiedStatus |
          | 2022 | 268.633333  | 7.2173904580668970625 | 5.8673804361619042791 | 2.2898520957463768590 | 14.8307257448787915306 | 268.6333333333333333333 | 79.4421957671957671957 | 1.033333                          |
    ...
    Scenario: Sending a valid school national average census history request with dimension Total, phase Primary, financeType Maintained
        ...
        When I submit the insights census request
        Then the census history result should be ok and contain:
          | Year | TotalPupils | Workforce | WorkforceHeadcount | Teachers   | TeachingAssistant | NonClassroomSupportStaff | AuxiliaryStaff | PercentTeacherWithQualifiedStatus |
          | 2022 | 308.019801  | 36.806930 | 47.376237          | 130.094059 | 1.004950          |                          | 3.410891       | 1.024752                          |
    ...
    Scenario: Sending a valid school national average census history request with dimension HeadcountPerFte, phase Primary, financeType Maintained
        ...
        When I submit the insights census request
        Then the census history result should be ok and contain:
          | Year | TotalPupils | Workforce             | WorkforceHeadcount    | Teachers              | TeachingAssistant      | AuxiliaryStaff        | PercentTeacherWithQualifiedStatus |
          | 2022 | 308.019801  | 1.2992935951022364958 | 1.0000000000000000000 | 0.0088508541685603504 | 16.3910891089108910891 | 1.0766867043847241867 | 1.024752                          |
    ...
    Scenario: Sending a valid school national average census history request with dimension PercentWorkforce, phase Primary, financeType Maintained
        ...
        Then the census history result should be ok and contain:
          | Year | TotalPupils | Workforce              | WorkforceHeadcount     | Teachers               | TeachingAssistant    | AuxiliaryStaff       | PercentTeacherWithQualifiedStatus |
          | 2022 | 308.019801  | 100.000000000000000000 | 100.000000000000000000 | 422.090092731097664571 | 3.234154307234142991 | 9.787430203141995055 | 1.024752                          |
    ...
    Scenario: Sending a valid school national average census history request with dimension PupilsPerStaffRole, phase Primary, financeType Maintained
        ...
        Then the census history result should be ok and contain:
          | Year | TotalPupils | Workforce             | WorkforceHeadcount    | Teachers              | TeachingAssistant       | AuxiliaryStaff         | PercentTeacherWithQualifiedStatus |
          | 2022 | 308.019801  | 8.4625263705554415226 | 6.6432385984097354055 | 2.4337378342010643450 | 307.3490099009900990099 | 99.4556793179317931793 | 1.024752                          |
    ...
    Scenario: Sending a valid school national average census history request with dimension Total, phase Secondary, financeType Academy
        ...
        Then the census history result should be ok and contain:
          | Year | TotalPupils | Workforce | WorkforceHeadcount | Teachers   | TeachingAssistant | NonClassroomSupportStaff | AuxiliaryStaff | PercentTeacherWithQualifiedStatus |
          | 2022 | 371.210000  | 47.000000 | 58.190000          | 127.440000 | 1.010000          |                          | 3.700000       | 1.020000                          |
    ...
    Scenario: Sending a valid school national average census history request with dimension HeadcountPerFte, phase Special, financeType Academy
        ...
        Then the census history result should be ok and contain:
          | Year | TotalPupils | Workforce             | WorkforceHeadcount    | Teachers              | TeachingAssistant      | NonClassroomSupportStaff | AuxiliaryStaff        | PercentTeacherWithQualifiedStatus |
          | 2022 | 348.829787  | 1.2830559767184912978 | 1.0000000000000000000 | 0.0088612061184511927 | 24.2340425531914893617 |                          | 1.0878465506125080593 | 1.074468                          |
