...
public class QueryHighNeedsHistoryFunctionTests : VersionedFunctionTestBase<IQueryHighNeedsHistoryHandler>
{
    private readonly QueryHighNeedsHistoryFunction _function;
    private readonly MockResponse _response;

    public QueryHighNeedsHistoryFunctionTests()
    {
        var identifier = Fixture.Create<string>();
        _response = new MockResponse(identifier);
        _function = new QueryHighNeedsHistoryFunction(Dispatcher.Object);
    }
