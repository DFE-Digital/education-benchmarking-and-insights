...
namespace Platform.Api.School.Features.Comparators.Services;

public interface IComparatorsService
{
    Task<ComparatorsResponse> ComparatorsAsync(string urn, ComparatorsRequest request, CancellationToken cancellationToken = default);
}
...
{
    public async Task<ComparatorsResponse> ComparatorsAsync(string urn, ComparatorsRequest request, CancellationToken cancellationToken = default)
    {
    ...
    private static double? CalculateScore(ComparatorsRequest request,
        ScoreResponse<ComparatorDto> x,
        ComparatorDto school)
    {
        if (x.Document == null)
        {
            return x.Score;
