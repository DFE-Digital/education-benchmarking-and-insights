...
public class ExpenditureQueryParametersValidator : AbstractValidator<ExpenditureQueryParameters>
{
    public ExpenditureQueryParametersValidator()
    {
        ...
        RuleFor(x => x.Urns)
            ...
            .WithMessage($"Either a collection of URNs or one of {nameof(ExpenditureQueryParameters.CompanyNumber)} or {nameof(ExpenditureQueryParameters.LaCode)} must be specified");

        RuleFor(x => x.LaCode)
            .Empty()
            .When(x => x.Urns.Length == 0)
            .When(x => !string.IsNullOrWhiteSpace(x.CompanyNumber))
            .WithMessage($"Either {nameof(ExpenditureQueryParameters.CompanyNumber)} or {nameof(ExpenditureQueryParameters.LaCode)} must be specified, not both");

        RuleFor(x => x.Phase)
            .Must(BeAValidPhase)
            .When(x => x.Urns.Length == 0)
            .When(x => !string.IsNullOrWhiteSpace(x.CompanyNumber) || !string.IsNullOrWhiteSpace(x.LaCode))
            .WithMessage($"'{{PropertyName}}' must be specified when {nameof(ExpenditureQueryParameters.CompanyNumber)} or {nameof(ExpenditureQueryParameters.LaCode)} is supplied and is not a recognized phase. Valid values are: {string.Join(", ", OverallPhase.All)}");
    }
