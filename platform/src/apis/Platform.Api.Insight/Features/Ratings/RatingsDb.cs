...
public class RatingsDb : IRatingsDb
{
    ...
    public async Task<IEnumerable<RatingResponseModel>> Get(RatingsQueryParameters query)
    {
        const string sql = "SELECT * from RAGRatings where URN IN @URNS AND PeerGroup = 'Default'";
        var parameters = new { URNS = query.Urns};
...
{
    Task<IEnumerable<RatingResponseModel>> Get(RatingsQueryParameters query);
}