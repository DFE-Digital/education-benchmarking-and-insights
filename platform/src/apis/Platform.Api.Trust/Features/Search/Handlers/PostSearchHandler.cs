...
public class PostSearchV1Handler(ITrustSearchService service, [FromKeyedServices(Constants.Features.Search)] IValidator<SearchRequest> validator) : IPostSearchHandler
{
    ...
    public async Task<HttpResponseData> HandleAsync(BasicContext context)
    {
        ...
        var trusts = await service.SearchAsync(body, context.Token);
