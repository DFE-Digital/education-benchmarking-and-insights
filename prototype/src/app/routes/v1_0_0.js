...
const router = govukPrototypeKit.requests.setupRouter("/v1_0_0");
...
router.get("/authority-homepage", (req, res) => {
  res.render(version + "/authority-homepage");
});

router.get("/trust-homepage", (req, res) => {
  res.render(version + "/trust-homepage");
});
...
router.get("/sign-in", (req, res) => {
  ...
  if (req.session.data["signedIn"] == "true") {
    ...
    res.render(version + "/sign-in", { goToUrl: goToUrl });
  }
});
...
router.get("/school-homepage", (req, res) => {
  res.render(version + "/school-homepage");
...
router.get("/comparators/create/characteristics", (req, res) => {
  ...
  res.render(version + "/comparators/create/characteristics", { rows: rows });
});
...
router.get("/comparators/create/preview", (req, res) => {
  ...
  if (
    req.session.data["comparators"] &&
    req.session.data["comparators"].length >= 30
  ) {
  ...
  for (i = 0; i < comparators.length; i++) {
    var nameHtml =
      ...
      comparators[i].comparatorName +
      '</span><br><span class="govuk-hint">' +
      comparators[i].comparatorLocation +
      ", " +
      comparators[i].comparatorPostcode +
      "</span>";
    var charsHtml =
      '<p class="govuk-body">Number of pupils</p><p class="govuk-body">Pupils with special educational needs (SEN):</p><p class="govuk-body">Number of schools within trust:</p><p class="govuk-body">Key stage 4 progress:</p>';
    var valuesHtml =
      '<p class="govuk-body">' +
      comparators[i].comparatorPupils.toLocaleString() +
      '</p><p class="govuk-body">' +
      comparators[i].comparatorSen +
      '%</p><p class="govuk-body">' +
      (Math.floor(Math.random() * (43 - 3 + 1)) + 3) +
      '</p><p class="govuk-body">' +
  ...
  res.render(version + "/comparators/create/preview", { rows: rows });
});
...
router.get("/comparators/view", (req, res) => {
  ...
  if (pupilComparators) {
    pupilComparators.sort((a, b) =>
      a.comparatorName > b.comparatorName ? 1 : -1
    );
    ...
    for (i = 0; i < pupilComparators.length; i++) {
      var nameHtml =
        '<a href="/' +
        version +
        "/comparators/view-school?comparatorType=pupil&comparatorId=" +
        [i] +
        '" class="govuk-link">' +
        pupilComparators[i].comparatorName +
        '</a><br><span class="govuk-hint">' +
        pupilComparators[i].comparatorLocation +
        ", " +
        pupilComparators[i].comparatorPostcode +
        "</span>";
      pupilRows.push([
        { html: nameHtml },
        { text: "Secondary" },
        { text: pupilComparators[i].comparatorPupils.toLocaleString() },
        { text: pupilComparators[i].comparatorSen + "%" },
        { text: pupilComparators[i].comparatorMeals + "%" },
      ]);
    }
  }

  if (buildingComparators) {
    buildingComparators.sort((a, b) =>
      a.buildingComparators > b.buildingComparators ? 1 : -1
    );
    ...
    for (i = 0; i < buildingComparators.length; i++) {
      var nameHtml =
        '<a href="/' +
        version +
        "/comparators/view-school?comparatorType=building&comparatorId=" +
        [i] +
        '" class="govuk-link">' +
        buildingComparators[i].comparatorName +
        '</a><br><span class="govuk-hint">' +
        buildingComparators[i].comparatorLocation +
        ", " +
        buildingComparators[i].comparatorPostcode +
        "</span>";
      var buildingRow = [
        { html: nameHtml },
        { text: "Secondary" },
        { text: buildingComparators[i].comparatorPupils.toLocaleString() },
      ];
      ...
      if (req.session.data["signedIn"] == "true") {
        buildingRow.push(
          {
            text:
              buildingComparators[i].comparatorGifa.toLocaleString() + " sqm",
          },
          { text: buildingComparators[i].comparatorAge + " years" }
        );
      } else if (i == 0) {
        buildingRow.push({
          html:
            '<a href="/sign-in?goTo=/' +
            version +
            '/comparators/view&hash=\&building">Sign in</a> to see',
          colspan: 2,
          rowspan: buildingComparators.length,
  ...
  res.render(version + "/comparators/view", {
    pupilRows: pupilRows,
...
router.post("/comparators/create", (req, res) => {
  ...
  res.redirect("/" + version + "/comparators/create/" + compareRoute);
});
...
router.get("/add-comparator-school", (req, res) => {
  ...
  res.redirect("/" + version + "/comparators/create/by-name");
});
...
router.get("/comparators/undo-remove", (req, res) => {
  ...
  res.redirect("/" + version + "/comparators/create/view");
});
...
router.get("/comparators/create/by-name", (req, res) => {
  ...
  for (i = 0; i < comparators.length; i++) {
    ...
    rows.push([
      ...
      { text: comparators[i].comparatorSen + "%" },
  ...
  res.render(version + "/comparators/create/by-name", {
    schoolRows: schoolRows,
    rows: rows,
    confirmation: req.session.data["confirmation"],
    comparatorSetType: req.session.data["comparatorSetType"],
...
router.get("/comparators/create/view", (req, res) => {
  ...
  for (i = 0; i < comparators.length; i++) {
    var nameHtml =
      '<a href="/' +
      version +
      "/comparators/view-school?comparatorType=custom&comparatorId=" +
      [i] +
      '" class="govuk-link">' +
      comparators[i].comparatorName +
      '</a><br><span class="govuk-hint">' +
  ...
  res.render(version + "/comparators/create/view", {
    schoolRows: schoolRows,
    rows: rows,
    confirmation: req.session.data["confirmation"],
    comparatorSetType: req.session.data["comparatorSetType"],
...
router.get("/comparators/view-school", (req, res) => {
  if (
    req.session.data.comparatorType == "pupil" &&
    req.session.data.pupilComparators
  ) {
    arrComparators = req.session.data.pupilComparators;
  } else if (
    req.session.data.comparatorType == "building" &&
    req.session.data.buildingComparators
  ) {
    arrComparators = req.session.data.buildingComparators;
  } else if (
    req.session.data.comparatorType == "custom" &&
    req.session.data.comparators
  ) {
  ...
  if (
    typeof arrComparators !== "undefined" &&
    arrComparators[req.session.data.comparatorId]
  ) {
    objSchool = arrComparators[req.session.data.comparatorId];
  } else {
    objSchool = {
      comparatorName: "Burngreave High School",
      comparatorLocation: "Sheffield",
      comparatorPostcode: "S13 9ZD",
      comparatorPupils: 1408,
      comparatorMeals: 14.7,
    };
  }

  res.render(version + "/comparators/view-school", {
    comparatorName: objSchool.comparatorName,
    comparatorLocation: objSchool.comparatorLocation,
    comparatorPostcode: objSchool.comparatorPostcode,
    comparatorPupils: objSchool.comparatorPupils.toLocaleString(),
    comparatorMeals: objSchool.comparatorMeals,
  });
});
...
router.post("/set-school", (req, res) => {
  if (req.session.data.signIn == "trust") {
    ...
    if (trustName) {
      req.session.data["trust-name"] = trustName.substring(
        0,
        trustName.lastIndexOf(" (")
      );
    }
    res.redirect("/" + version + "/trust-homepage");
  } else if (req.session.data.signIn == "authority") {
    var authorityName = req.session.data.authority;
    if (authorityName) {
      req.session.data["authority-name"] = authorityName.substring(
        0,
        authorityName.lastIndexOf(" (")
      );
    }
    res.redirect("/" + version + "/authority-homepage");
  } else {
    var schoolName = req.session.data.school;
    if (schoolName) {
...
router.post("/compare-trusts", (req, res) => {
  ...
  res.redirect("/" + version + "/compare-trusts/" + compareRouteTrusts);
});
...
router.get("/compare-trusts/by-name", (req, res) => {
  ...
  for (i = 0; i < trusts.length; i++) {
    ...
    var nameHtml =
      ...
      trusts[i].trustName +
      '</span><br><span class="govuk-hint">' +
      trusts[i].trustNumber +
      "</span>";
    rows.push([
      { html: nameHtml },
      { html: schoolTypes },
      {
        text: trusts[i].trustPupils.toLocaleString(),
        classes: "govuk-!-text-align-right",
      },
      {
        text: "£" + trusts[i].trustIncome.toLocaleString(),
        classes: "govuk-!-text-align-right",
      },
      {
        html:
  ...
  res.render(version + "/compare-trusts/by-name", {
    trustRows: trustRows,
    rows: rows,
    confirmation: req.session.data["confirmation"],
    trustSetType: req.session.data["trustSetType"],
    errorThisPage: req.session.data["errorThisPage"],
    errorNoSchool: req.session.data["errorNoSchool"],
  });
...
router.get("/compare-trusts/view", (req, res) => {
  ...
  res.render(version + "/compare-trusts/view", {
    trustRows: trustRows,
    rows: rows,
    confirmation: req.session.data["confirmation"],
    trustSetType: req.session.data["trustSetType"],
...
router.get("/compare-trusts/preview", (req, res) => {
  ...
  for (i = 0; i < trusts.length; i++) {
    var nameHtml =
      ...
      "</span>";
    var charsHtml =
      '<p class="govuk-body">Number of pupils:</p><p class="govuk-body">Number of schools:</p><p class="govuk-body">Trust income:</p><p class="govuk-body">Trust formation year:</p>';
    var valuesHtml =
      '<p class="govuk-body">' +
      trusts[i].trustPupils.toLocaleString() +
      '</p><p class="govuk-body">' +
      (Math.floor(Math.random() * (43 - 3 + 1)) + 3) +
      '</p><p class="govuk-body">£' +
      trusts[i].trustIncome.toLocaleString() +
      '</p><p class="govuk-body">' +
      (Math.floor(Math.random() * (2024 - 2011 + 1)) + 2011) +
      "</p>";
    rows.push([{ html: nameHtml }, { html: charsHtml }, { html: valuesHtml }]);
  }

  res.render(version + "/compare-trusts/preview", { rows: rows });
});
...
function addTrust(req, res, trustName) {
  ...
  trustNumber = trustName.substring(
    trustName.lastIndexOf(" (") + 2,
    trustName.length - 1
  );
  trustSchools = Math.floor(Math.random() * (18 - 4.3 + 1) + 4.3);
  trustSchoolsPrimary = Math.floor(trustSchools * Math.random());
  trustSchoolsNursery = Math.floor(
    (trustSchools - trustSchoolsPrimary) * Math.random()
  );
  trustSchoolsSecondary =
    trustSchools - trustSchoolsPrimary - trustSchoolsNursery;
  trustPupils =
    Math.floor(Math.random() * (2782 - 438 + 1)) + 438 * trustSchools;
  trustIncome = Math.floor(
    (Math.random() * (18 - 4.3 + 1) + 4.3) * 1000000 * trustSchools
  );

  trusts.unshift({
    trustName: trustNameOnly,
    trustNumber: trustNumber,
    trustPupils: trustPupils,
    trustSchools: trustSchools,
    trustSchoolsPrimary: trustSchoolsPrimary,
    trustSchoolsNursery: trustSchoolsNursery,
    trustSchoolsSecondary: trustSchoolsSecondary,
...
router.get("/compare-trusts/add-trust", (req, res) => {
  ...
  res.redirect("/" + version + "/compare-trusts/by-name");
});

router.get("/compare-trusts/remove", (req, res) => {
  if (req.session.data.trusts) {
    req.session.data["trust-removed"] =
      req.session.data.trusts[req.query.id].trustName;
...
router.get("/compare-trusts/reset-confirmed", (req, res) => {
  ...
  res.render(version + "/trust-homepage", { confirmation: "trust-reset" });
});
...
router.get("/custom-data/generated", (req, res) => {
  ...
  res.render(version + "/custom-data/custom-homepage", {
...
function getSchoolList() {
  var objSchoolsFile = require("../data/schools.json");
  ...
  for (i = 0; i < objSchools.length; i++) {
    ...
    schools.push({
      text:
        objSchool.schoolName +
        " (" +
        objSchool.schoolLocation +
        ", " +
        objSchool.schoolPostcode +
        ")",
    });
  }
...
function getTrustList() {
  var objTrustsFile = require("../data/trusts.json");
  var objTrusts = objTrustsFile.trusts;
  var trusts = [];

  for (i = 0; i < objTrusts.length; i++) {
    trusts.push({
      text: objTrusts[i].trustName + " (" + objTrusts[i].trustNumber + ")",
    });
  }
...
function getLocalAuthorityList() {
  var objAuthorityFile = require("../data/authorities.json");
  ...
  for (i = 0; i < objAuthorities.length; i++) {
    ...
    lcoalAuthorities.push({
      text:
        objAuthority.authorityName + " (" + objAuthority.authorityCode + ")",
    });
  }
...
function generatePupilComparators() {
  ...
  for (i = 0; i < 30; i++) {
    ...
    comparators.push({
      comparatorName: objSchool.schoolName,
      comparatorLocation: objSchool.schoolLocation,
      comparatorPostcode: objSchool.schoolPostcode,
      comparatorPupils: comparatorPupils,
      comparatorMeals: comparatorMeals,
      comparatorSen: comparatorSen,
    });
...
function generateBuildingComparators() {
  ...
  for (i = 0; i < 30; i++) {
    ...
    comparatorPupils = Math.floor(Math.random() * (2782 - 438 + 1)) + 438;
    comparatorGifa =
      comparatorPupils * Math.floor(Math.random() * (12 - 10 + 1) + 10);
    ...
    comparators.push({
      ...
      comparatorPostcode: objSchool.schoolPostcode,
      comparatorPupils: comparatorPupils,
      comparatorAge: comparatorAge,
      comparatorGifa: comparatorGifa,
    });
  }
...
function generateTrusts() {
  ...
  for (i = 0; i < 10; i++) {
    ...
    trusts.push({
      trustName: objTrust.trustName,
      trustNumber: objTrust.trustNumber,
      trustPupils: trustPupils,
      trustSchools: trustSchools,
      trustSchoolsPrimary: trustSchoolsPrimary,
      trustSchoolsNursery: trustSchoolsNursery,
...
router.get("*/manage-prototype-data/view-data", function (req, res) {
  ...
  res.render("manage-prototype-data/view-data", {
