...
router.get( '/comparators/create', (req, res) => {
    ...
    for ( i=0; i<comparators.length; i++) {
        rows.push( [ {'text': comparators[i].comparatorName}, {'text': 'Secondary'}, {'text': comparators[i].comparatorPupils.toLocaleString()}, {'text': 'Good'}, {'text': comparators[i].comparatorMeals + '%'}, {'html': '<a href="/' + version + '/comparators/remove?id=' + i + '">Remove</a>' } ] );
    }

    res.render( version + '/comparators/create', { rows: rows, confirmation: req.session.data['confirmation'], errorThisPage: req.session.data['errorThisPage'], errorNoSchool: req.session.data['errorNoSchool'] } );
