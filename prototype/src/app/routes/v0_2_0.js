...
const router = govukPrototypeKit.requests.setupRouter('/v0_2_0');

const version = 'v0_2_0';
...
router.get( '/find-school', (req, res) => {
    ...
    res.render( version + '/find-school', { rows: rows } );
})

router.get( '/comparators/create/local-authority', (req, res) => {

    var rows = getLocalAuthorityList();
    res.render( version + '/comparators/create/local-authority', { rows: rows } );
})
...
router.post( '/comparators/create/review', (req, res) => {
    ...
    res.redirect( '/' + version + '/comparators/create/review' );
...
router.get( '/comparators/pupil', (req, res) => {
    ...
    res.render( version + '/comparators/pupil', { rows: rows } );
})
...
router.get( '/comparators/building', (req, res) => {
    ...
    res.render( version + '/comparators/building', { rows: rows } );
})

router.post( '/comparators/create', (req, res) => {

    res.redirect( '/' + version + '/comparators/create/' + req.session.data['compareRoute'] );
...
router.get( '/comparators/create/review', (req, res) => {
    ...
    for ( i=0; i<comparators.length; i++) {
        var nameHtml = "<a href=\"\" class=\"govuk-link\">" + comparators[i].comparatorName +"</a><br><span class=\"govuk-hint\">" + comparators[i].comparatorLocation + ", " + comparators[i].comparatorPostcode + "</span>";
        rows.push( [ {'html':  nameHtml}, {'text': 'Secondary'}, {'text': comparators[i].comparatorPupils.toLocaleString()}, {'text': 'Good'}, {'text': comparators[i].comparatorMeals + '%'}, {'html': '<a href="/' + version + '/comparators/remove?id=' + i + '">Remove</a>' } ] );
    }

    res.render( version + '/comparators/create/review', { schoolRows: schoolRows, rows: rows, confirmation: req.session.data['confirmation'], errorThisPage: req.session.data['errorThisPage'], errorNoSchool: req.session.data['errorNoSchool'] } );
