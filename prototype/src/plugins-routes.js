...
function setupPathsFor(item) {
  plugins.getPublicUrlAndFileSystemPaths(item).forEach((paths) => {
    ...
    requests.serveDirectory(
      paths.publicUrl.replace("plugin-assets", "extension-assets"),
      paths.fileSystemPath
    );

    //Fix for azure
    requests.serveDirectory(
      paths.publicUrl.replace("%40x-govuk%2F", "@x-govuk/"),
      paths.fileSystemPath
    );
  });
}
