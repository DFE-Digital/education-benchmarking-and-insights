...
def test_trust_rollup(academies, central_services, tolerance=0.001):
    ...
    academy_cs_totals = (
        academies[academies["Total Expenditure_CS"].notna()]  # Only academies with CS data
        .groupby("Trust Name")
    ...
    cs_totals = (
        central_services[["Company_Name", "Total Expenditure"]]
        .rename(columns={"Company_Name": "Trust Name"})
    )
    ...
    comparison["Difference"] = comparison["Academy_CS_Total"] - comparison["Total Expenditure"]
    ...
    if not all_within_tolerance:
        ...
        print(outliers[["Trust Name", "Academy_CS_Total", "Total Expenditure", "Difference"]].to_string(index=False))
    
    return all_within_tolerance


def test_academies_rollup(academies, aar, tolerance = 0.01):
    comparison_academies = academies[["URN"]].assign(
        Derived_No_CS=academies["Total Expenditure"] - academies["Total Expenditure_CS"]
    ).merge(
    ...
    comparison_academies["Within_Tolerance"] = (
        (comparison_academies["Derived_No_CS"] - comparison_academies["Original_AAR"]) <= tolerance
    )
