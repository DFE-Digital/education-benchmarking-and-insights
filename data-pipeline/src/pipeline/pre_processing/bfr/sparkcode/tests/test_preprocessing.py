...
def test_preprocess_bfr_sofa_scaling(preprocessor, spark_session):
    ...
    pupil_number_row = processed_df.filter(processed_df["Trust UPIN"] == 100001).filter(processed_df["EFALineNo"] == config.SOFA_PUPIL_NUMBER_EFALINE).first()
    ...
    it_spend_row = processed_df.filter(processed_df["Trust UPIN"] == 100001).filter(processed_df["EFALineNo"] == 336).first()
...
def test_preprocess_bfr_sofa_aggregation(preprocessor, spark_session):
    ...
    self_gen_income_row = processed_df.filter(upin_mask).filter(processed_df["Category"] == "Self-generated income").first()
    ...
    grant_funding_agg_row = processed_df.filter(upin_mask).filter(processed_df["Category"] == "Grant funding").first()
...
def test_preprocess_bfr_sofa_category_rename(preprocessor, spark_session):
    ...
    revenue_reserve_row = processed_df.filter(processed_df["Trust UPIN"] == 100001).filter(processed_df["EFALineNo"] == config.SOFA_TRUST_REVENUE_RESERVE_EFALINE).first()
    ...
    pupil_numbers_row = processed_df.filter(processed_df["Trust UPIN"] == 100001).filter(processed_df["EFALineNo"] == config.SOFA_PUPIL_NUMBER_EFALINE).first()
...
def test_preprocess_bfr_3y_normalization_and_scaling(preprocessor, spark_session):
    ...
    normalized_income_row = processed_df.filter(processed_df["Trust UPIN"] == 100001).filter(processed_df["EFALineNo"] == config.SOFA_TOTAL_REVENUE_INCOME).first()
    assert normalized_income_row is not None
    assert normalized_income_row["Y2"] == 100.0 * 1000 # Value from mock_bfr_three_year_mv_data for 2980 (Y2)

    # Original 3Y EFALine 4300 maps to SOFA_TRUST_REVENUE_RESERVE_EFALINE (430)
    normalized_reserve_row = processed_df.filter(processed_df["Trust UPIN"] == 100001).filter(processed_df["EFALineNo"] == config.SOFA_TRUST_REVENUE_RESERVE_EFALINE).first()
    assert normalized_reserve_row is not None
    assert normalized_reserve_row["Y2"] == 200.0 * 1000 # Value from mock_bfr_three_year_mv_data for 4300 (Y2)

    # Check a line that doesn't need mapping (OTHER_COSTS_EFALINE = 335) and is scaled
    other_costs_row = processed_df.filter(processed_df["Trust UPIN"] == 100001).filter(processed_df["EFALineNo"] == config.OTHER_COSTS_EFALINE).first()
    assert other_costs_row is not None
    assert other_costs_row["Y2"] == 50.0 * 1000 # Value from mock_bfr_three_year_mv_data for 335 (Y2)
...
def test_preprocess_data_merge_and_crn(preprocessor, spark_session):
    ...
    mock_bfr_three_year_mv_data = get_mock_bfr_three_year_mv(spark_session)
    mock_bfr_sofa_mv_data = get_mock_bfr_sofa_mv(spark_session, year=2024)
    merged_df = preprocessor.preprocess_data(mock_bfr_sofa_mv_data, mock_bfr_three_year_mv_data, mock_academies_data)
    ...
    expected_columns = [
        "Company Registration Number", "Trust UPIN", "EFALineNo", "Category",
        *expected_sofa_year_cols, *expected_3y_cols
    ]
    ...
    sofa_and_3y_row = merged_df.filter((merged_df["Trust UPIN"] == 100001) & (merged_df["EFALineNo"] == config.SOFA_TOTAL_REVENUE_INCOME)).first()
    assert sofa_and_3y_row is not None
    assert sofa_and_3y_row["Y1P1"] == 1000.0 * 1000 # SOFA value scaled
    assert sofa_and_3y_row["Y2"] == 100.0 * 1000 # 3Y value scaled and normalized (Y2 in 3Y maps to first year of projection)
    ...
    pupil_row = merged_df.filter(
        (merged_df["Trust UPIN"] == 100001) & (merged_df["EFALineNo"] == config.SOFA_PUPIL_NUMBER_EFALINE)).first()
    assert pupil_row is not None
    assert pupil_row["Y1P1"] == 1000.0 # Not scaled (from SOFA mock)
    assert pupil_row["Y2"] == 1000.0 # Not scaled (from 3Y mock, originally 9000, mapped to 999)
    ...
    it_spend_sofa_only_row = merged_df.filter(
        (merged_df["Trust UPIN"] == 100001) & (merged_df["EFALineNo"] == 336)).first()
