...
from pyspark.sql.functions import (
    col, when, sum as spark_sum, avg, collect_list, size, 
    isnan, isnull, lit, row_number, desc, asc, regexp_replace,
    array, explode, monotonically_increasing_id, percent_rank
)
...
def calculate_metrics(bfr: DataFrame) -> DataFrame:
    ...
    bfr_with_metric = bfr.withColumn(
        "metric",
        when(col("Category").isin(costs_list), col("Y2P1") + col("Y2P2"))
        .otherwise(col("Y2P2"))
    )
    
    # Pivot the data to get categories as columns
    df_pivot = bfr_with_metric.groupBy("Company Registration Number").pivot("Category").sum("metric")
    
    # Calculate percentage metrics
    df_with_percentages = df_pivot.withColumn(
    ...
    df_melted = df_with_percentages.select(
        ...
        *[col(c) for c in percentage_columns]
    )
    
    # Unpivot using stack function
    stack_expr = f"stack({len(percentage_columns)}, " + \
    ...
    df_final = df_unpivoted.withColumn(
        "Value",
        when(isnan(col("Value")) | isnull(col("Value")) | 
             (col("Value") == float('inf')) | (col("Value") == float('-inf')), 0.0)
        .otherwise(col("Value"))
    )
...
def calculate_slopes_spark(df: DataFrame, year_columns: list) -> DataFrame:
    ...
    df_with_arrays = df.withColumn(
        "y_values", 
        array([col(c) for c in year_columns])  # Keep original nulls/NaNs
    ).withColumn(
        "x_values",
        array([lit(x) for x in x_values])
    )
...
def slope_analysis(bfr: DataFrame) -> DataFrame:
    ...
    df_melted = df_with_flags.select(
        col("Company Registration Number"),
        col("Trust UPIN"),
        F.expr("stack(2, 'Slope', Slope, 'Slope flag', `Slope flag`) as (Category, value)")
    )
