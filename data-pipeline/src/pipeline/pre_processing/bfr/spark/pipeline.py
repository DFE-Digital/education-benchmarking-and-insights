...
import os
from src.bfr.logging import setup_logger
from src.bfr.engine import build_bfr_data, build_bfr_historical_data
...
def hash_compare_view_dfs(table_id: str, view: DataFrame, materialized_view: DataFrame) -> bool:
    """
...
class DatabricksFBITPipeline(ABC):
    ...
    @staticmethod
    def check_for_updates_in_materialized_views(
        comparison_tuples: list[tuple[str, DataFrame, DataFrame]]
    ):
        """
        ...
        Args:
            comparison_tuples: list of tuples containing the table id, source view, and materialized view
        """
        for table_id, source_view, materialized_view in comparison_tuples:
            try:
...
class BFRPipeline(DatabricksFBITPipeline):
    ...
    def run(self):
        ...
        bfr_final, bfr_metrics_final = self.transform_data(bfr_sofa_mv, bfr_three_year_mv)
        self.save_data(bfr_final, bfr_metrics_final)
    
    def load_data(self):
        """Load raw materialized views for BFR data"""
        bfr_sofa_mv = self.spark.table("catalog_40_copper_financial_benchmarking_insights.financial_benchmarking_insights.mv_BFR_Sofa_2025")
        bfr_three_year_mv = self.spark.table("catalog_40_copper_financial_benchmarking_insights.financial_benchmarking_insights.mv_BFR_Three_Year_Forecast_2025")
        bfr_sofa_source_table = self.spark.table("catalog_30_bronze.bfr.vw_BFR_Sofa_2024")
        bfr_three_year_source_table = self.spark.table("catalog_30_bronze.bfr.vw_BFR_Three_Year_Forecast_2024")

        comparison_tuples = [
            ("bfr_sofa", bfr_sofa_source_table, bfr_sofa_mv),
            ("bfr_three_year", bfr_three_year_source_table, bfr_three_year_mv)
        ]
        self.check_for_updates_in_materialized_views(comparison_tuples)
    ...
    def load_ancillary_data(self):
        ...
        self.bfr_sofa_year_minus_one, self.bfr_sofa_year_minus_two = self._get_historical_bfr_data()
    ...
    def _get_historical_bfr_data(self):
        ...
        self.bfr_sofa_year_minus_one = self.spark.table(f"catalog_30_bronze.bfr.vw_BFR_Sofa_{last_year}") \
                                                .select(historical_bfr_columns) \
                                                .withColumnRenamed("TrustUPIN", "Trust UPIN") \
                                                .filter(col("EFALineNo").isin(desired_efa_lines))
        self.bfr_sofa_year_minus_two = self.spark.table(f"catalog_30_bronze.bfr.vw_BFR_Sofa_{year_before_last}") \
                                                .select(historical_bfr_columns) \
                                                .withColumnRenamed("TrustUPIN", "Trust UPIN") \
                                                .filter(col("EFALineNo").isin(desired_efa_lines))
    ...
    def _get_academies_data(self):
        ...
        logger.info(f"Processing BFR academies data: default/{self.year}/academies.parquet")
        academies_path = f"{preprocessed_path}/default/{self.year}/academies.parquet"
        academies = (spark.read.parquet(academies_path)
                    .select(
                        "Trust UPIN",
        ...
        logger.info(f"Academies preprocessed {self.year=} shape: {academies.count()} rows")
    ...
    def _get_historical_academies_data(self):  
        ...
        academies_y1_path = f"{preprocessed_path}/default/{self.year - 1}/academies.parquet"
        ...
        academies_y2 = None
        academies_y2_path = f"{preprocessed_path}/default/{self.year - 2}/academies.parquet"
