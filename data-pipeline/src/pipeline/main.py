...
def pre_process_data(
    ...
    if academies_ilr_data := pre_process_ilr_data(
        run_type,
        run_id,
        aar_year,
        schools=academies_data_ref[0],
    ):
        academies = patch_missing_sixth_form_data(
            df=academies,
            ilr=academies_ilr_data,
            gias_links=academies_data_ref[9],
        )

    if maintained_ilr_data := pre_process_ilr_data(
        run_type,
        run_id,
        cfr_year,
        schools=maintained_data_ref[0],
    ):
        maintained_schools = patch_missing_sixth_form_data(
            df=maintained_schools,
            ilr=maintained_ilr_data,
            gias_links=maintained_data_ref[0],
        )
