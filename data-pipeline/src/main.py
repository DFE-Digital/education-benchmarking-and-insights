...
def pre_process_data(set_type, year):
    ...
    write_blob('pre-processed', f'{set_type}/{year}/cdc.csv', cdc.to_csv(encoding='utf-8'))

    logger.info("Processing Census Data")
    workforce_census_data = get_blob(raw_container, f'{set_type}/{year}/census_workforce.xlsx')
    pupil_census_data = get_blob(raw_container, f'{set_type}/{year}/census_pupils.csv', encoding='utf-8')
    census = pre_processing.prepare_census_data(workforce_census_data, pupil_census_data)
    write_blob('pre-processed', f'{set_type}/{year}/census.csv', census.to_csv(encoding='utf-8'))
    ...
    write_blob('pre-processed', f'{set_type}/{year}/sen.csv', sen.to_csv(encoding='utf-8'))
    ...
    write_blob('pre-processed', f'{set_type}/{year}/ks2.csv', ks2.to_csv(encoding='utf-8'))
    ...
    write_blob('pre-processed', f'{set_type}/{year}/ks4.csv', ks4.to_csv(encoding='utf-8'))
    ...
    write_blob('pre-processed', f'{set_type}/{year}/trust_ar.csv', trust_ar.to_csv(encoding='utf-8'))
    write_blob('pre-processed', f'{set_type}/{year}/academy_ar.csv', academy_ar.to_csv(encoding='utf-8'))

    logger.info("Processing Schools Data")
    gias_data = get_blob(raw_container, f'{set_type}/{year}/gias.csv', encoding="cp1252")
    gias_links_data = get_blob(raw_container, f'{set_type}/{year}/gias_links.csv', encoding="cp1252")
    schools = pre_processing.prepare_schools_data(gias_data, gias_links_data)
    write_blob('pre-processed', f'{set_type}/{year}/schools.csv', schools.to_csv(encoding='utf-8'))

    logger.info("Building Academy Set")
    academies_data = get_blob(raw_container, f'{set_type}/{year}/academy_master_list.csv', encoding='utf-8')
    academies = pre_processing.build_academy_data(academies_data, year, schools, census, sen, cdc, academy_ar, trust_ar, ks2, ks4)
    write_blob('pre-processed', f'{set_type}/{year}/academies.csv', academies.to_csv(encoding='utf-8'))

    logger.info("Building Maintained School Set")
    maintained_schools_data = get_blob(raw_container, f'{set_type}/{year}/maintained_schools_master_list.csv', encoding='utf-8')
    maintained_schools = pre_processing.build_maintained_school_data(maintained_schools_data, year, schools, census, sen, cdc, ks2, ks4)
    write_blob('pre-processed', f'{set_type}/{year}/maintained_schools.csv', maintained_schools.to_csv(encoding='utf-8'))

    logger.info("Building Federations Set")
    gias_all_links_data = get_blob(raw_container, f'{set_type}/{year}/gias_all_links.csv', encoding="unicode-escape")
    (hard_federations, soft_federations) = pre_processing.build_federations_data(gias_all_links_data, maintained_schools)
    write_blob('pre-processed', f'{set_type}/{year}/hard_federations.csv', hard_federations.to_csv(encoding='utf-8'))
    write_blob('pre-processed', f'{set_type}/{year}/soft_federations.csv', soft_federations.to_csv(encoding='utf-8'))
...
def compute_comparator_sets(set_type, year):
    ...
    pp_academy_data = get_blob('pre-processed', f'{set_type}/{year}/academies.csv', encoding="utf-8")
    pp_maintained_schools_data = get_blob('pre-processed', f'{set_type}/{year}/maintained_schools.csv', encoding="utf-8")
    academy_data = comparators.prepare_data(pd.read_csv(pp_academy_data))
    ms_data = comparators.prepare_data(pd.read_csv(pp_maintained_schools_data, low_memory=False))

    logger.info("Writing all schools data")
    all_schools = pd.concat([academy_data, ms_data])
    write_blob('comparator-sets', f'{set_type}/{year}/schools.pkl', pickle.dumps(all_schools, protocol=pickle.HIGHEST_PROTOCOL))
    ...
    write_blob('comparator-sets', f'{set_type}/{year}/ms_pupil.pkl', pickle.dumps(ms_pupil_comparators, protocol=pickle.HIGHEST_PROTOCOL))
    ...
    write_blob('comparator-sets', f'{set_type}/{year}/ms_building.pkl', pickle.dumps(ms_building_comparators, protocol=pickle.HIGHEST_PROTOCOL))
    ...
    write_blob('comparator-sets', f'{set_type}/{year}/academy_pupil.pkl', pickle.dumps(academy_pupil_comparators, protocol=pickle.HIGHEST_PROTOCOL))
    ...
    write_blob('comparator-sets', f'{set_type}/{year}/academy_building.pkl',
    ...
    write_blob('comparator-sets', f'{set_type}/{year}/all_pupil.pkl',
    ...
    write_blob('comparator-sets', f'{set_type}/{year}/all_building.pkl',
...
def receive_messages():
    try:
        ...
        if msg is not None:
            ...
            try:
                msg_payload['pre_process_duration'] = pre_process_data(msg_payload['type'], msg_payload['year'])
                msg_payload['comparator_set_duration'] = compute_comparator_sets(msg_payload['type'], msg_payload['year'])
