...
def prepare_census_data(workforce_census_path, pupil_census_path):
    school_workforce_census = pd.read_excel(workforce_census_path, header=5,
                                            index_col=schemas.workforce_census_index_col,
                                            usecols=schemas.workforce_census.keys(), dtype=schemas.workforce_census,
                                            na_values=["x", "u", "c"], keep_default_na=True).drop_duplicates()

    school_pupil_census = pd.read_csv(pupil_census_path, encoding='utf8',
                                      index_col=schemas.pupil_census_index_col, usecols=schemas.pupil_census.keys(),
                                      dtype=schemas.pupil_census).drop_duplicates()

    census = school_pupil_census.join(school_workforce_census, on='URN', how='inner', rsuffix='_pupil',
                                      lsuffix='_workforce')
...
def prepare_sen_data(sen_path):
    sen = pd.read_csv(sen_path, encoding='cp1252', index_col=schemas.sen_index_col, dtype=schemas.sen,
    ...
    sen.rename(columns={'prov_slcn': 'Prov_SLCN', 'prov_hi': 'Prov_HI', 'prov_vi': 'Prov_VI', 'prov_msi': 'Prov_MSI',
                        'prov_pd': 'Prov_PD', 'prov_asd': 'Prov_ASD', 'prov_oth': 'Prov_OTH'}, inplace=True)

    return sen[
        ['Total pupils', 'EHC plan', 'Percentage SEN', 'Primary Need SPLD', 'Primary Need MLD', 'Primary Need SLD',
         'Primary Need PMLD', 'Primary Need SEMH', 'Primary Need SLCN', 'Primary Need HI', 'Primary Need VI',
         'Primary Need MSI', 'Primary Need PD', 'Primary Need ASD', 'Primary Need OTH', 'Prov_SPLD', 'Prov_MLD',
         'Prov_SLD', 'Prov_PMLD', 'Prov_SEMH', 'Prov_SLCN', 'Prov_HI', 'Prov_VI', 'Prov_MSI', 'Prov_PD', 'Prov_ASD',
         'Prov_OTH']]
...
def prepare_ks4_data(ks4_path):
    ...
    return ks4[['URN', 'AverageAttainment', 'Progress8Measure', 'Progress8Banding']].copy()
...
def prepare_aar_data(aar_path):
    ...
    trust_financial_position = trust_financial[['trustupin', 'Academy Balance']].groupby('trustupin').sum().rename(
        columns={'Academy Balance': 'Trust Balance'})
    ...
    central_services_financial_position = central_services_financial[['trustupin', 'Academy Balance']].groupby(
        'trustupin').sum().rename(columns={'Academy Balance': 'Central Services Balance'})
...
def prepare_schools_data(base_data_path, links_data_path):
    gias = pd.read_csv(base_data_path, encoding='cp1252',
                       index_col=schemas.gias_index_col, usecols=schemas.gias.keys(), dtype=schemas.gias)

    gias_links = pd.read_csv(links_data_path, encoding='cp1252',
                             index_col=schemas.gias_links_index_col, usecols=schemas.gias_links.keys(),
...
def build_academy_data(academy_data_path, year,
                       schools, census, sen, cdc, academy_ar, trust_agg, ks2, ks4):
    ...
    academies_list = pd.read_csv(academy_data_path, encoding='utf8',
                                 index_col=schemas.academy_master_list_index_col, dtype=schemas.academy_master_list,
                                 usecols=schemas.academy_master_list.keys()).rename(columns={'UKPRN': 'Academy UKPRN'})

    academies_base = academies_list.merge(schools.reset_index(), left_index=True,
                                          right_on='LA Establishment Number').set_index('URN')
    ...
    academies['Type of Provision - Phase'] = academies.apply(
        lambda df: mappings.map_academy_phase_type(df['TypeOfEstablishment (code)'], df['Type of Provision - Phase']),
        axis=1)
    ...
    academies['NurseryProvision (name)'] = academies.apply(
        lambda df: mappings.map_nursery(df['NurseryProvision (name)'], df['Type of Provision - Phase']), axis=1)

    academies['Status'] = academies.apply(
        lambda df: mappings.map_academy_status(pd.to_datetime(df['Date left or closed if in period']),
                                               ...
                                               pd.to_datetime(academy_year_end_date)), axis=1)

    academies['SchoolPhaseType'] = academies.apply(
        lambda df: mappings.map_school_phase_type(df['TypeOfEstablishment (code)'], df['Type of Provision - Phase']),
        axis=1)
...
def build_maintained_school_data(maintained_schools_data_path, year, schools, census, sen, cdc, ks2, ks4):
    maintained_schools_year_start_date = datetime.date(year, 4, 1)
    maintained_schools_year_end_date = datetime.date(year, 3, 31)

    maintained_schools_list = pd.read_csv(maintained_schools_data_path, encoding='utf8',
    ...
    maintained_schools['Status'] = maintained_schools.apply(
        lambda df: mappings.map_maintained_school_status(df['OpenDate'], df['CloseDate'],
                                                         df['Period covered by return (months)'],
                                                         pd.to_datetime(maintained_schools_year_start_date),
                                                         pd.to_datetime(maintained_schools_year_end_date)), axis=1)
    maintained_schools['School Balance'] = maintained_schools['Total Income   I01 to I18'] - maintained_schools[
        'Total Expenditure  E01 to E32']
    maintained_schools['School Financial Position'] = maintained_schools['School Balance'].map(
        mappings.map_is_surplus_deficit)
    maintained_schools['SchoolPhaseType'] = maintained_schools.apply(
        lambda df: mappings.map_school_phase_type(df['TypeOfEstablishment (code)'], df['Overall Phase']), axis=1)
    maintained_schools['Partial Years Present'] = maintained_schools['Period covered by return (months)'].map(
        lambda x: x != 12)
...
def build_federations_data(links_data_path, maintained_schools):
    group_links = pd.read_csv(links_data_path, encoding='unicode-escape',
                              index_col=schemas.groups_index_col, usecols=schemas.groups.keys(), dtype=schemas.groups)
