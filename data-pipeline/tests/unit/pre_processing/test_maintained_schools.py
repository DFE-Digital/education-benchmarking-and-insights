...
def test_create_master_list(maintained_schools_master_list: pd.DataFrame,
    ...
    assert {'Percentage SEN', 'Number of pupils', 'Age Average Score', 'Ks2Progress', 'Progress8Measure'}.issubset(list(actual.columns))
    # Assert DNS is replaced
    assert len(actual[actual.eq("DNS").any(axis=1)]) == 0


def test_map_status(maintained_schools_master_list: pd.DataFrame,
                            prepared_schools_data: pd.DataFrame,
                            prepared_sen_data_df: pd.DataFrame,
                            prepared_census_data: pd.DataFrame,
                            prepared_cdc_data_df: pd.DataFrame,
...
def test_calc_base_financials(maintained_schools_master_list: pd.DataFrame, column, expected):
    actual = maintained_schools.calc_base_financials(maintained_schools_master_list).iloc[0]
    assert actual[column] == expected


def test_map_cost_income_categories(maintained_schools_master_list: pd.DataFrame):
    income_mappings = {
        "I03  SEN funding": "Income_SEN"
    }
    ...
    actual = maintained_schools.map_cost_income_categories(maintained_schools_master_list, cost_mappings, income_mappings)

    assert actual.columns.isin(["Income_SEN"]).any()
    assert actual.columns.isin(["Catering staff and supplies_Catering staff"]).any()


def test_calc_rag_cost_series(maintained_schools_master_list: pd.DataFrame,
                            prepared_schools_data: pd.DataFrame,
                            prepared_sen_data_df: pd.DataFrame,
                            prepared_census_data: pd.DataFrame,
                            prepared_cdc_data_df: pd.DataFrame,
    ...
    assert actual.columns.isin(["Teaching and Teaching support staff_Teaching staff_Per Unit"]).any()
    assert actual.columns.isin(["Teaching and Teaching support staff_Total"]).any()
    assert actual.columns.isin(["Premises staff and services_Maintenance of premises_Per Unit"]).any()
    ...
    assert value_row["Teaching and Teaching support staff_Teaching staff_Per Unit"] == (1000 / 320)
    assert value_row["Premises staff and services_Total"] == 1000
    assert value_row["Premises staff and services_Maintenance of premises_Per Unit"] == (1000 / 2803.0)
