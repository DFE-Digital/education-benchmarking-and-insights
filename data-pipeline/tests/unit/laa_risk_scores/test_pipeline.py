...
def test_preprocess_laa_extra_ancillary_data():
    ...
    parental_preference_raw = pd.DataFrame({
        ...
        "total_number_places_offered": [40.0, 100.0, 0.0]  # One with zero offers to test division-by-zero
    })
    ...
    school_1001_capacity = capacity[capacity["school_urn"] == 1001]["school_places"].values[0]
    ...
    school_1001_pref = parental_pref[parental_pref["school_urn"] == 1001]["proportion_1stprefs_v_totaloffers"].values[0]
    assert school_1001_pref == 1.25  # 50.0 / 40.0
    
    # Check that division by zero was handled safely and returned 0.0
    school_1003_pref = parental_pref[parental_pref["school_urn"] == 1003]["proportion_1stprefs_v_totaloffers"].values[0]
...
def test_preprocess_laa_data_merges_and_aligns_urn_index():
    ...
    cfr_this = pd.DataFrame({
        "Lead school in federation": ["0", "1", "0"],
        "Value": [100.0, 200.0, 300.0]
    }, index=[1001, 1002, 1003])
    cfr_this.index.name = "URN"

    cfr_minus_one = pd.DataFrame({
        "Value": [3000.0, 1000.0, 2000.0]
    }, index=[1003, 1001, 1002]) # different order
    ...
    absences = pd.DataFrame({
        "school_urn": [1001, 1003],
        "sess_overall_percent": [4.0, 5.0]
    })
    capacity = pd.DataFrame({
        "school_urn": [1001, 1003],
        "school_places": [120.0, 180.0]
    })
    parental_pref = pd.DataFrame({
        "school_urn": [1001, 1003],
        "proportion_1stprefs_v_totaloffers": [1.25, 0.5]
    })
...
def test_orchestrator_pipeline(mock_load_ancillary, mock_load_cfr):
    ...
    mock_load_ancillary.return_value = {
        "absences_raw": pd.DataFrame({"time_period": [202425], "school_urn": [1001], "sess_overall_percent": [4.0]}),
        "capacity_raw": pd.DataFrame({"time_period": [202425], "school_urn": [1001], "school_places": [120.0]}),
        "capacity_special_raw": pd.DataFrame({"time_period": [202425], "school_urn": [1001], "school_places": [10.0]}),
        "parental_preference_raw": pd.DataFrame({"time_period": [202425], "school_urn": [1001], "times_put_as_1st_preference": [1.0], "total_number_places_offered": [1.0]}),
        "run_year": 2025
    }
