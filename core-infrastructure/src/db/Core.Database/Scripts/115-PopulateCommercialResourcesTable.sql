-- this script is intended for one time use (May 2025) to populate the 
-- CommercialResources table with the initial data
-- to be used as part of the release to each environment.

IF EXISTS(SELECT *
              FROM INFORMATION_SCHEMA.TABLES
              WHERE table_name = 'CommercialResources')
BEGIN
    IF NOT EXISTS(SELECT 1 FROM CommercialResources)
    BEGIN
