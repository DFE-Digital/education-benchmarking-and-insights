...
| <a name="input_configuration"></a> [configuration](#input\_configuration) | n/a | <pre>map(object({<br>    sql-db = object({<br>      sku_name    = string<br>      max_size_gb = number<br>    })<br>  }))</pre> | <pre>{<br>  "automated-test": {<br>    "sql-db": {<br>      "max_size_gb": 5,<br>      "sku_name": "S0"<br>    }<br>  },<br>  "development": {<br>    "sql-db": {<br>      "max_size_gb": 5,<br>      "sku_name": "S0"<br>    }<br>  },<br>  "feature": {<br>    "sql-db": {<br>      "max_size_gb": 5,<br>      "sku_name": "S0"<br>    }<br>  },<br>  "pre-production": {<br>    "sql-db": {<br>      "max_size_gb": 5,<br>      "sku_name": "S0"<br>    }<br>  },<br>  "production": {<br>    "sql-db": {<br>      "max_size_gb": 5,<br>      "sku_name": "S0"<br>    }<br>  },<br>  "test": {<br>    "sql-db": {<br>      "max_size_gb": 5,<br>      "sku_name": "S0"<br>    }<br>  }<br>}</pre> | no |
