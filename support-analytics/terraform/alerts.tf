...
resource "azurerm_monitor_metric_alert" "chart_rendering_api_cpu_alert" {
  name                = "chart-rendering-api-cpu-alert"
  resource_group_name = azurerm_resource_group.resource-group.name
  scopes              = [data.azurerm_windows_function_app.chart-rendering-api.id]
  description         = "Alert if CPU utilisation exceeds ${var.configuration[var.environment].thresholds.cpu}% for more than 5 minutes"
  severity            = 2
  frequency           = "PT1M"
  window_size         = "PT5M"
...
resource "azurerm_monitor_metric_alert" "chart_rendering_api_memory_alert" {
  name                = "chart-rendering-api-memory-alert"
  resource_group_name = azurerm_resource_group.resource-group.name
  scopes              = [data.azurerm_windows_function_app.chart-rendering-api.id]
  description         = "Alert if memory utilisation exceeds ${var.configuration[var.environment].thresholds.memory}% for more than 5 minutes"
  severity            = 2
