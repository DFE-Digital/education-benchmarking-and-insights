...
def academies_current_year():
    ...
    spark = SparkSession.builder.getOrCreate()
    current_year = int(spark.conf.get("pipeline.year"))
    return get_mock_academies_df(spark, current_year)
...
def academies_historical():
    ...
    current_year = int(spark.conf.get("pipeline.year"))
    historical_year = current_year - 1
    return get_mock_academies_df(spark, historical_year)
...
@dp.table(comment="Mocked")
def academies_y1_processed():
    return dp.read("academies_historical")
