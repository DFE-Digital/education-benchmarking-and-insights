...
from education_benchmarking_and_insights.bfr.forecast_and_risk_tables import (
    _build_bfr_historical_data,
    _calculate_slopes,
    _assign_slope_flag,
)
...
# No longer need fixtures for calculator classes since we're using functions directly


def test_calculate_slopes(spark):
    ...
    result_df = _calculate_slopes(bfr_df, year_columns)
...
def test_assign_slope_flag(spark):
    # Prepare input data for _assign_slope_flag with multiple slopes per company
    data = [
        {"Company_Number": "C1", "Slope": 0.1},
        {"Company_Number": "C1", "Slope": -0.5},
        {"Company_Number": "C1", "Slope": 1.0},
        {"Company_Number": "C1", "Slope": -2.0},
        {"Company_Number": "C1", "Slope": 0.0},
        {"Company_Number": "C1", "Slope": 0.5},
        {"Company_Number": "C1", "Slope": -0.1},
        {"Company_Number": "C1", "Slope": 0.2},
        {"Company_Number": "C1", "Slope": -0.8},
        {"Company_Number": "C1", "Slope": 1.5},
        {"Company_Number": "C2", "Slope": 10.0},
        {"Company_Number": "C2", "Slope": 20.0},
        {"Company_Number": "C2", "Slope": 30.0},
        {"Company_Number": "C2", "Slope": 40.0},
        {"Company_Number": "C2", "Slope": 50.0},
    ]
    bfr_with_slopes_df = spark.createDataFrame(data)

    # Call the function under test
    result_df = _assign_slope_flag(bfr_with_slopes_df)
    result_pdf = result_df.toPandas()
...
def test_build_bfr_historical_data_academies_none(spark):
    # Test case where academies_historical is None
    bfr_sofa_historical_data = [
        {"TrustUPIN": "0", "EFALineNo": 430, "Y1P2": 2048.0, "Y2P2": 1024.0}
    ]
    bfr_sofa_historical_df = spark.createDataFrame(bfr_sofa_historical_data)

    result = _build_bfr_historical_data(
...
def test_build_bfr_historical_data_bfr_sofa_none(spark):
    # Test case where bfr_sofa_historical is None
    academies_historical_data = [
        {
            "TrustUPIN": "0",
            "Company_Number": "0",
            "TotalPupilsInTrust": 100,
        }
    ]
    academies_historical_df = spark.createDataFrame(academies_historical_data)

    result_df = _build_bfr_historical_data(
        academies_historical=academies_historical_df,
        bfr_sofa_historical=None,
    )
    result_pdf = result_df.toPandas()

    assert "TrustRevenueReserve" in result_pdf.columns
    assert list(result_pdf["TrustRevenueReserve"]) == [0.0]
    assert "TotalPupilsInTrust" in result_pdf.columns
    assert list(result_pdf["TotalPupilsInTrust"]) == [0]


def test_build_bfr_historical_data_both_none():
...
def test_build_bfr_historical_data_empty_bfr_sofa(spark):
    ...
    empty_bfr_sofa_schema = StructType(
        [
            StructField("TrustUPIN", StringType(), True),
    ...
    assert list(result_pdf["TrustRevenueReserve"]) == [0.0]
    assert "TotalPupilsInTrust" in result_pdf.columns
    assert result_pdf["TotalPupilsInTrust"].tolist() == [100]


def test_build_bfr_historical_data_full_data(spark):
    ...
    bfr_sofa_historical_data = [
        ...
        {"TrustUPIN": "0", "EFALineNo": 1, "Y1P2": 500.0, "Y2P2": 0.0},  # Pupil number
    ]
    bfr_sofa_historical_df = spark.createDataFrame(bfr_sofa_historical_data)
    ...
    assert result_pdf["TrustRevenueReserve"].iloc[0] == 1_024_000.0
    assert "TotalPupilsInTrust" in result_pdf.columns
    assert result_pdf["TotalPupilsInTrust"].iloc[0] == 100
